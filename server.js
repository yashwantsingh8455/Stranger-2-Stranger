...
io.on("connection", socket => {
  ...
  socket.on("join", async data => {
    try {
      ...
      let firebaseUid = null, firebaseEmail = null, guestId = null, guestCountry = null, authType = null;
      if (firebaseToken) {
      ...
      currentUser = {
        ...
        firebaseUid, firebaseEmail, guestId, country: guestCountry, authType, authId,
        isAdmin,
...
app.post("/api/guest/register", async (req, res) => {
  ...
  const guest = await GuestUser.create({
    ...
    sessionTokenHash, expiresAt, lastSeen: new Date(),
  });
