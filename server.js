...
const REQUIRE_EMAIL_VERIFIED = process.env.REQUIRE_EMAIL_VERIFIED !== "false";
...
function buildUserList() {
  return Object.values(activeUsers)
    ...
    .map(u => ({
      ...
      isVip:    !!u.isVip,
      isAdmin:  !!u.isAdmin,
      status:   u.presenceStatus || "online",
...
function buildGroupUserList(groupId) {
  ...
  return Object.values(room).map(u => ({ uid: u.firebaseUid || "", name: u.name, bio: u.bio, isVip: !!u.isVip, isAdmin: !!u.isAdmin, status: u.presenceStatus || "online", customStatus: u.customStatus || "" }));
}
...
function publicGroup(group) {
  ...
  return {
    ...
    slowModeSeconds: Number(g.slowModeSeconds || 0),
    announcementsOnly: !!g.announcementsOnly,
...
function isAllowedMediaPayload(value, type = "text") {
  if (typeof value !== "string") return false;
  if (!value.startsWith("data:")) return true; // external URL / normal text; URL sanitization happens client-side too
  if (value.length > 2_000_000) return false; // ~1.5 MiB decoded cap
  const mime = (value.match(/^data:([^;,]+)[;,]/i) || [])[1]?.toLowerCase() || "";
  const allowed = {
    image: ["image/jpeg", "image/png", "image/webp", "image/gif"],
    gif: ["image/gif", "image/jpeg", "image/png", "image/webp"],
...
function canUserSeeGroup(group, user) {
  if (!group || !user) return false;
  if ((group.visibility || "public") !== "private") return true;
  if (user.isAdmin) return true;
  if (group.adminUid && user.firebaseUid && group.adminUid === user.firebaseUid) return true;
  return !!(user.firebaseUid && (group.roles || []).some(r => r.uid === user.firebaseUid));
}

async function emitGroupsListToAll() {
  ...
  for (const user of Object.values(activeUsers)) {
    io.to(user.socketId).emit("groups_list", groups.filter(g => canUserSeeGroup(g, user)).map(publicGroup));
  }
}
...
io.on("connection", socket => {
  ...
  socket.on("join", async data => {
    try {
      ...
      if (REQUIRE_FIREBASE_AUTH && !firebaseAdminReady) {
        return socket.emit("auth_error", "Authentication service is not configured on the server.");
      }
      if (REQUIRE_FIREBASE_AUTH && !firebaseToken) {
        return socket.emit("auth_error", "Please login again before entering chat.");
      }

      let firebaseUid = null, firebaseEmail = null;
      if (firebaseToken) {
        const decoded = await verifyFirebaseToken(firebaseToken);
        if (!decoded) return socket.emit("auth_error", "❌ Invalid or expired session. Please login again.");
        firebaseUid = decoded.uid;
        firebaseEmail = decoded.email || null;
        if (REQUIRE_EMAIL_VERIFIED && decoded.email && !decoded.email_verified) return socket.emit("auth_error", "Please verify your email before entering chat.");
        ...
        if (savedProfile) {
          ...
          presenceStatus = savedProfile.presenceStatus || "online";
          customStatus = savedProfile.customStatus || "";
      ...
      const duplicate = Object.values(activeUsers).find(u =>
        u.name.toLowerCase() === nameLower && (!firebaseUid || u.firebaseUid !== firebaseUid)
      );
      if (duplicate) return socket.emit("duplicate", "⚠️ Ye username already liya hua hai.");

      // Reconnect of the same authenticated account replaces its old socket cleanly.
      if (firebaseUid) {
        const old = Object.values(activeUsers).find(u => u.firebaseUid === firebaseUid && u.socketId !== socket.id);
        if (old) {
      ...
      if (mongoConnected) {
        try {
          await UserSession.findOneAndUpdate(
            { firebaseUid: firebaseUid || undefined, username: nameLower },
            {
          ...
          await UserSession.updateOne({ firebaseUid: firebaseUid || undefined, username: nameLower }, { $inc: { sessionCount: 1 } });
      ...
      socket.emit("history", history.map(m => ({
        id: m._id.toString(), senderUid: m.senderId || "", sender: m.senderName, senderColor: m.senderColor, senderAvatar: m.senderAvatar,
        message: m.text, type: m.type||"text", mediaUrl: m.mediaUrl, caption: m.caption||"", replyTo: m.replyTo||null,
        isVip: m.isVip, createdAt: m.createdAt, reactions: m.reactions||[], editedAt:m.editedAt||null, pinned:!!m.pinned, expiresAt:m.expiresAt||null, forwardedFrom:m.forwardedFrom||null, language:m.language||"unknown",
      })));
      ...
      socket.emit("joined", { ...currentUser, ip: undefined, firebaseEmail: undefined });
      updateDiscordStatus();
  ...
  socket.on("chat message", async data => {
    ...
    try {
      ...
      const type = ["text", "image", "video", "voice", "audio", "gif", "file"].includes(String(data.type || "text")) ? String(data.type || "text") : "text";
      ...
      if (type !== "text" && !isAllowedMediaPayload(message || mediaUrl, type)) return socket.emit("error_msg", "Media type invalid hai ya file ~1.5 MB se badi hai.");

      // Global rate limiting + repeated-message protection.
      const nowMs = Date.now();
      currentUser._messageTimes = (currentUser._messageTimes || []).filter(t => nowMs - t < 10000);
      if (currentUser._messageTimes.length >= 10) return socket.emit("error_msg", "Slow down — max 10 messages per 10 seconds.");
      currentUser._messageTimes.push(nowMs);
      if (type === "text") {
        const normalized = message.trim().toLowerCase();
        if (normalized && currentUser._lastMessageText === normalized && nowMs - (currentUser._lastMessageAt || 0) < 8000) {
          return socket.emit("error_msg", "Duplicate message blocked to reduce spam.");
        }
        currentUser._lastMessageText = normalized;
        currentUser._lastMessageAt = nowMs;
      }

      const moderation = type === "text" && typeof socialFeatures !== "undefined" ? socialFeatures.helpers.autoModerate(message) : { flagged:false, score:0, reasons:[] };
      if (moderation.score >= 60) return socket.emit("error_msg", "Message blocked by the safety filter.");
      ...
      const expiresAt = [300, 3600, 86400, 604800].includes(expiresIn) ? new Date(Date.now() + expiresIn * 1000) : undefined;
      const forwardedFrom = data.forwardedFrom && typeof data.forwardedFrom === "object" ? {
        messageId: String(data.forwardedFrom.messageId || "").slice(0, 64),
        sender: String(data.forwardedFrom.sender || "").slice(0, 30),
      } : undefined;
      const language = type === "text" && typeof socialFeatures !== "undefined" ? socialFeatures.helpers.detectLanguage(message) : "unknown";

      const saved = await safeDB(() => new Message({
        room, senderId: currentUser.firebaseUid || socket.id, senderName: currentUser.name, senderAvatar: currentUser.avatar,
        senderColor: currentUser.color, text: message, type, mediaUrl, caption, replyTo, isVip: currentUser.isVip,
        expiresAt, forwardedFrom, language, moderation,
      }).save(), null);

      const payload = {
        id: saved?._id?.toString() || data.id || genId(), senderUid: currentUser.firebaseUid || "", sender: currentUser.name, senderColor: currentUser.color,
        ...
        expiresAt: saved?.expiresAt || expiresAt || null, forwardedFrom: saved?.forwardedFrom || forwardedFrom || null, language, moderation, reactions: [], editedAt: null, pinned: false,
      };
      ...
      if (type === "text" && typeof socialFeatures !== "undefined") {
        const lowerText = message.toLowerCase();
        const mentioned = new Set();
        for (const u of Object.values(activeUsers)) {
          if (u.firebaseUid && u.firebaseUid !== currentUser.firebaseUid && lowerText.includes(`@${String(u.name || "").toLowerCase()}`)) mentioned.add(u.firebaseUid);
        }
        for (const uid of mentioned) await socialFeatures.helpers.notify(uid, "mention", `${currentUser.name} mentioned you in chat.`, { messageId: payload.id, room, fromUid: currentUser.firebaseUid || "" });
      }
  ...
  // ── PRIVATE MESSAGE (legacy UI, now governed by connection/block rules) ─────────
  socket.on("private message", async data => {
    ...
    try {
      ...
      const type = ["text", "image", "video", "voice", "audio", "gif", "file"].includes(data.type) ? data.type : "text";
      const text = String(data.message || "").slice(0, 4000);
      const caption = String(data.caption || "").slice(0, 500);
      const mediaUrl = String(data.mediaUrl || "");
      if (type === "text" && !text.trim()) return;
      if (type !== "text" && !isAllowedMediaPayload(mediaUrl || text, type)) return socket.emit("error_msg", "Media is invalid or exceeds the 1.5 MB limit.");

      const escapedName = receiverName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      const toUser = Object.values(activeUsers).find(u => u.name.toLowerCase() === receiverName.toLowerCase());
      const targetProfile = await safeDB(() => UserProfile.findOne({ displayName: { $regex: new RegExp(`^${escapedName}$`, "i") } }).lean(), null);
      const receiverUid = toUser?.firebaseUid || targetProfile?.firebaseUid;
      if (!receiverUid) return socket.emit("error_msg", "User profile not found.");
      if (await socialFeatures.helpers.isBlockedEitherWay(currentUser.firebaseUid, receiverUid)) return socket.emit("error_msg", "Messaging is unavailable for this connection.");
      if (!(await socialFeatures.helpers.acceptedConnection(currentUser.firebaseUid, receiverUid))) return socket.emit("error_msg", "Permanent DMs require an accepted connection.");

      const channelId = [currentUser.firebaseUid, receiverUid].sort().join("__");
      ...
      dmDoc.messages.push({
        senderUid: currentUser.firebaseUid, receiverUid, senderName: currentUser.name, senderAvatar: currentUser.avatar, senderColor: currentUser.color,
        text, type, mediaUrl, caption, createdAt: new Date(), deliveredAt: toUser ? new Date() : undefined,
      });
      ...
      const payload = {
        channelId, id: saved?._id?.toString() || genId(), sender: currentUser.name, senderUid: currentUser.firebaseUid,
        senderAvatar: currentUser.avatar, senderColor: currentUser.color, receiver: receiverName, receiverUid,
        message: text, type, mediaUrl, caption, createdAt: saved?.createdAt || new Date(), deliveredAt: saved?.deliveredAt || null,
      };
      socket.emit("private message", payload);
      if (toUser) io.to(toUser.socketId).emit("private message", payload);
      else await socialFeatures.helpers.notify(receiverUid, "dm", `${currentUser.name} sent you a message.`, { fromUid: currentUser.firebaseUid });
  ...
  socket.on("dm_history", async ({ withUser } = {}) => {
    if (!currentUser?.firebaseUid) return;
    const receiverName = String(withUser || "").trim();
    const escapedName = receiverName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const targetProfile = await safeDB(() => UserProfile.findOne({ displayName: { $regex: new RegExp(`^${escapedName}$`, "i") } }).lean(), null);
    if (!targetProfile?.firebaseUid || !(await socialFeatures.helpers.acceptedConnection(currentUser.firebaseUid, targetProfile.firebaseUid)) || await socialFeatures.helpers.isBlockedEitherWay(currentUser.firebaseUid, targetProfile.firebaseUid)) {
      return socket.emit("dm_history_data", { channelId: "", withUser: receiverName, messages: [] });
    }
    const channelId = [currentUser.firebaseUid, targetProfile.firebaseUid].sort().join("__");
    const dmDoc = await safeDB(() => DM.findOne({ channelId }), null);
    let changed = false;
    if (dmDoc) {
      for (const m of dmDoc.messages || []) if (m.receiverUid === currentUser.firebaseUid && !m.readAt) { m.readAt = new Date(); changed = true; }
      if (changed) await safeDB(() => dmDoc.save());
    }
    const messages = (dmDoc?.messages || []).map(m => ({
      id: m._id?.toString(), sender: m.senderName, senderUid: m.senderUid, senderAvatar: m.senderAvatar, senderColor: m.senderColor, message: m.text,
      type: m.type, mediaUrl: m.mediaUrl, caption: m.caption||"", createdAt: m.createdAt, deliveredAt:m.deliveredAt||null, readAt:m.readAt||null, editedAt:m.editedAt||null, reactions:m.reactions||[],
    }));
    socket.emit("dm_history_data", { channelId, withUser: receiverName, messages });
  });

  // ── DM TYPING ────────────────────────────────────────────────────
  socket.on("dm_typing", async ({ toUser, isTyping } = {}) => {
    if (!currentUser?.firebaseUid) return;
    const target = Object.values(activeUsers).find(u => u.name.toLowerCase() === String(toUser || "").toLowerCase());
    if (!target?.firebaseUid) return;
    if (!(await socialFeatures.helpers.acceptedConnection(currentUser.firebaseUid, target.firebaseUid)) || await socialFeatures.helpers.isBlockedEitherWay(currentUser.firebaseUid, target.firebaseUid)) return;
    io.to(target.socketId).emit("dm_typing_update", { fromUser: currentUser.name, isTyping: !!isTyping });
  });
  ...
  // ── CREATE GROUP ─────────────────────────────────────────────────
  socket.on("create_group", async ({ name, description, password, icon, visibility = "public", joinApproval = false, rules = [], category = "General", slowModeSeconds = 0, announcementsOnly = false, isCommunity = false } = {}) => {
    if (!currentUser) return;
    try {
      ...
      const group = new Group({
        ...
        adminName: currentUser.name, adminUid: currentUser.firebaseUid || "", icon: String(icon || "👥").slice(0, 10), members: [currentUser.name],
        visibility: ["public", "private"].includes(visibility) ? visibility : "public",
        joinApproval: !!joinApproval,
        rules: Array.isArray(rules) ? rules.map(r => String(r).slice(0,200)).filter(Boolean).slice(0,20) : [],
        category: String(category || "General").slice(0,40),
        slowModeSeconds: Math.max(0, Math.min(3600, Number(slowModeSeconds) || 0)),
        announcementsOnly: !!announcementsOnly,
        isCommunity: !!isCommunity,
        roles: currentUser.firebaseUid ? [{ uid: currentUser.firebaseUid, name: currentUser.name, role: "owner" }] : [],
      });
      await safeDB(() => group.save());
      socket.emit("group_created", publicGroup({ ...group.toObject(), passwordHash }));
      await emitGroupsListToAll();
  ...
  socket.on("get_groups", async () => {
    if (!currentUser) return;
    const groups = await safeDB(() => Group.find({}).select("+password +passwordHash").sort({ createdAt: -1 }).lean(), []);
    socket.emit("groups_list", groups.filter(g => canUserSeeGroup(g, currentUser)).map(publicGroup));
  });
  ...
  socket.on("join_group", async ({ groupId, password } = {}) => {
    ...
    try {
      ...
      const isGroupOwner = !!(group.adminUid && currentUser.firebaseUid && group.adminUid === currentUser.firebaseUid);
      const existingRole = currentUser.firebaseUid ? group.roles?.find(r => r.uid === currentUser.firebaseUid) : null;
      if ((group.visibility || "public") === "private" && !isGroupOwner && !existingRole && !currentUser.isAdmin) {
        return socket.emit("group_error", "This private group requires a valid invite and membership approval.");
      }
      const memberCount = Math.max((group.roles || []).length, (group.members || []).length);
      if (!existingRole && !isGroupOwner && Number(group.maxMembers || 0) > 0 && memberCount >= Number(group.maxMembers)) {
        return socket.emit("group_error", "This group is full.");
      }
      if (!isGroupOwner && group.joinApproval && !existingRole) {
        if (currentUser.firebaseUid && !(group.pendingMembers || []).some(r => r.uid === currentUser.firebaseUid)) {
          group.pendingMembers.push({ uid: currentUser.firebaseUid, name: currentUser.name, requestedAt: new Date() });
          await safeDB(() => group.save());
        }
        return socket.emit("group_join_pending", { groupId, message: "Join request sent for moderator approval." });
      }
      let passwordOk = isGroupOwner || !!existingRole;
      ...
      if (currentUser.firebaseUid && !(group.roles || []).some(r => r.uid === currentUser.firebaseUid)) {
        group.roles.push({ uid: currentUser.firebaseUid, name: currentUser.name, role: isGroupOwner ? "owner" : "member" });
        groupChanged = true;
      }
      if (groupChanged) await safeDB(() => group.save());
      ...
      const normalizedHistory = history.map(m => ({
        id: m._id.toString(), senderUid: m.senderId || "", sender: m.senderName, senderColor: m.senderColor, senderAvatar: m.senderAvatar, message: m.text,
        type: m.type||"text", mediaUrl: m.mediaUrl, caption: m.caption||"", replyTo: m.replyTo||null, isVip: m.isVip, createdAt: m.createdAt, reactions:m.reactions||[], editedAt:m.editedAt||null, pinned:!!m.pinned, expiresAt:m.expiresAt||null, forwardedFrom:m.forwardedFrom||null, language:m.language||"unknown",
      }));
  ...
  socket.on("leave_group", async ({ groupId } = {}) => {
    ...
    socket.emit("history", globalHistory.map(m => ({
      id: m._id.toString(), senderUid: m.senderId || "", sender: m.senderName, senderColor: m.senderColor, senderAvatar: m.senderAvatar,
      message: m.text, type: m.type||"text", mediaUrl: m.mediaUrl, caption: m.caption||"", replyTo: m.replyTo||null, isVip: m.isVip, createdAt: m.createdAt, reactions:m.reactions||[], editedAt:m.editedAt||null, pinned:!!m.pinned, expiresAt:m.expiresAt||null, forwardedFrom:m.forwardedFrom||null, language:m.language||"unknown",
    })));
  });
  ...
  socket.on("group_message", async data => {
    ...
    try {
      ...
      if (type !== "text" && !isAllowedMediaPayload(message || mediaUrl, type)) return socket.emit("group_error", "Media type invalid hai ya file ~1.5 MB se badi hai.");
      ...
      const groupDoc = await safeDB(() => Group.findById(groupId).lean(), null);
      const role = groupDoc && currentUser.firebaseUid ? (groupDoc.adminUid === currentUser.firebaseUid ? "owner" : groupDoc.roles?.find(r => r.uid === currentUser.firebaseUid)?.role) : null;
      if (groupDoc?.announcementsOnly && !["owner","admin","moderator"].includes(role) && !currentUser.isAdmin) return socket.emit("group_error", "This is an announcements-only channel.");
      const slowSeconds = Number(groupDoc?.slowModeSeconds || 0);
      if (slowSeconds > 0 && !["owner","admin","moderator"].includes(role) && !currentUser.isAdmin) {
        currentUser._groupLastMessage = currentUser._groupLastMessage || {};
        const last = currentUser._groupLastMessage[groupId] || 0;
        const remaining = slowSeconds * 1000 - (Date.now() - last);
        if (remaining > 0) return socket.emit("group_error", `Slow mode: wait ${Math.ceil(remaining/1000)}s.`);
        currentUser._groupLastMessage[groupId] = Date.now();
      }
      const moderation = type === "text" && typeof socialFeatures !== "undefined" ? socialFeatures.helpers.autoModerate(message) : { flagged:false, score:0, reasons:[] };
      if (moderation.score >= 60) return socket.emit("group_error", "Message blocked by the safety filter.");
      ...
      const expiresIn = Number(data.expiresIn || 0);
      const expiresAt = [300, 3600, 86400, 604800].includes(expiresIn) ? new Date(Date.now() + expiresIn * 1000) : undefined;
      const forwardedFrom = data.forwardedFrom && typeof data.forwardedFrom === "object" ? { messageId: String(data.forwardedFrom.messageId || "").slice(0,64), sender: String(data.forwardedFrom.sender || "").slice(0,30) } : undefined;
      const language = type === "text" && typeof socialFeatures !== "undefined" ? socialFeatures.helpers.detectLanguage(message) : "unknown";
      const saved = await safeDB(() => new Message({
        room: roomName, senderId: currentUser.firebaseUid || socket.id, senderName: currentUser.name, senderAvatar: currentUser.avatar,
        senderColor: currentUser.color, text: message, type, mediaUrl, caption, replyTo, isVip: currentUser.isVip, expiresAt, forwardedFrom, language, moderation,
      }).save(), null);
      const payload = { id: saved?._id?.toString() || genId(), groupId, senderUid: currentUser.firebaseUid || "", sender: currentUser.name, senderColor: currentUser.color, senderAvatar: currentUser.avatar, isVip: currentUser.isVip, message, type, mediaUrl, caption, replyTo, createdAt: saved?.createdAt || new Date(), expiresAt:saved?.expiresAt||expiresAt||null, forwardedFrom:saved?.forwardedFrom||forwardedFrom||null, language, moderation, reactions:[], editedAt:null, pinned:false };
      io.to(roomName).emit("group_message", payload);
      if (type === "text" && typeof socialFeatures !== "undefined") {
        const lowerText = message.toLowerCase();
        const mentioned = new Set();
        for (const u of Object.values(activeGroups[groupId] || {})) {
          if (u.firebaseUid && u.firebaseUid !== currentUser.firebaseUid && lowerText.includes(`@${String(u.name || "").toLowerCase()}`)) mentioned.add(u.firebaseUid);
        }
        for (const uid of mentioned) await socialFeatures.helpers.notify(uid, "mention", `${currentUser.name} mentioned you in ${groupDoc?.name || "a group"}.`, { messageId: payload.id, room: roomName, groupId, fromUid: currentUser.firebaseUid || "" });
      }
  ...
  socket.on("delete_group", async ({ groupId } = {}) => {
    ...
    try {
      ...
      if (!group) return socket.emit("group_error", "Group nahi mili.");
      const isOwner = (group.adminUid && currentUser.firebaseUid && group.adminUid === currentUser.firebaseUid) ||
  ...
  socket.on("update profile", async ({ bio, avatar, color, name } = {}) => {
    ...
    if (currentUser.firebaseUid && nextName !== currentUser.name) {
      const profileForCooldown = await safeDB(() => UserProfile.findOne({ firebaseUid: currentUser.firebaseUid }).lean(), null);
      const changedAt = profileForCooldown?.usernameChangedAt ? new Date(profileForCooldown.usernameChangedAt).getTime() : 0;
      const cooldownMs = 30 * 24 * 60 * 60 * 1000;
      if (changedAt && Date.now() - changedAt < cooldownMs) return socket.emit("error_msg", `Username can be changed again in ${Math.ceil((cooldownMs - (Date.now()-changedAt))/86400000)} day(s).`);
    }
    ...
    if (currentUser.firebaseUid && mongoConnected) {
      const dbDup = await UserProfile.findOne({ displayName: { $regex: new RegExp("^" + nextName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "$", "i") }, firebaseUid: { $ne: currentUser.firebaseUid } }).lean();
      if (dbDup) return socket.emit("error_msg", "Username already taken.");
    }

    const usernameWasChanged = nextName !== currentUser.name;
    ...
    if (currentUser.firebaseUid) {
      await safeDB(() => UserProfile.findOneAndUpdate(
        ...
        { $set: { email: currentUser.firebaseEmail, displayName: currentUser.name, bio: currentUser.bio, avatar: currentUser.avatar, color: currentUser.color, updatedAt: new Date(), ...(usernameWasChanged ? { usernameChangedAt: new Date() } : {}) } },
        { upsert: true }
      ));
    }
    io.emit("user list", buildUserList());
...
async function requireFirebaseUser(req, res, next) {
  ...
  if (REQUIRE_EMAIL_VERIFIED && decoded.email && !decoded.email_verified) return res.status(403).json({ error: "Please verify your email first" });
...
app.put("/api/profile", requireFirebaseUser, async (req, res) => {
  ...
  const currentProfile = await UserProfile.findOne({ firebaseUid: decoded.uid }).lean();
  const usernameWasChanged = !!currentProfile && currentProfile.displayName && currentProfile.displayName !== displayName;
  if (usernameWasChanged && currentProfile.usernameChangedAt) {
    const cooldownMs = 30 * 24 * 60 * 60 * 1000;
    const elapsed = Date.now() - new Date(currentProfile.usernameChangedAt).getTime();
    if (elapsed < cooldownMs) return res.status(429).json({ error: `Username can be changed again in ${Math.ceil((cooldownMs-elapsed)/86400000)} day(s).` });
  }
  ...
  const existingConflict = await UserProfile.findOne({ displayName: { $regex: new RegExp("^" + displayName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "$", "i") }, firebaseUid: { $ne: decoded.uid } }).lean();
  if (existingConflict) return res.status(409).json({ error: "That username is already taken" });
  const profile = await UserProfile.findOneAndUpdate(
    { firebaseUid: decoded.uid },
    { $set: { email: decoded.email || "", displayName, bio, avatar, color, location, theme, updatedAt: new Date(), ...(usernameWasChanged ? { usernameChangedAt: new Date() } : {}) }, $setOnInsert: { createdAt: new Date() } },
    { upsert: true, returnDocument: "after" }
  ).lean();
...
// ══════════════════════════════════════════════════════════════════
// 🌐 SOCIAL V2 — matching, connections, privacy, XP, groups, safety
// ══════════════════════════════════════════════════════════════════
...
async function startServer() {
  ...
  http.listen(PORT, () => {
    ...
    console.log(`🔐 Firebase auth: ${REQUIRE_FIREBASE_AUTH ? "REQUIRED" : "OPTIONAL (development only)"}`);
    console.log(`📧 Email verification: ${REQUIRE_EMAIL_VERIFIED ? "REQUIRED" : "OPTIONAL"}`);
