// ╔══════════════════════════════════════════════════════════════════╗
// ║   StrangerToStranger — HeyyYuki Powered Server v6.2             ║
// ║   Firebase + Guest Auth + Moderation + Cashfree Admin Pass      ║
// ║   Reports + Analytics + Groups + Social Discovery | 2026       ║
// ╚══════════════════════════════════════════════════════════════════╝
...
// JSON parsing is installed once, before all API modules. Cashfree webhook
// signature verification needs the untouched raw request body, so keep a copy
// only for that endpoint while still exposing req.body to the route.
app.use(express.json({
  limit: "2mb",
  verify: (req, _res, buf) => {
    if (req.originalUrl?.startsWith("/api/paid-admin/webhook")) {
...
const MsgSchema = new mongoose.Schema({
  ...
  isPaidAdmin:  { type: Boolean, default: false },
...
function buildUserList() {
  return Object.values(activeUsers).map(u => ({
    ...
    isVip:       !!u.isVip,
    isAdmin:     !!u.isAdmin,
    isPaidAdmin: !!u.isPaidAdmin,
  }));
}

function buildGroupUserList(groupId) {
  const room = activeGroups[groupId] || {};
  return Object.values(room).map(u => ({ name: u.name, bio: u.bio, isVip: !!u.isVip, isAdmin: !!u.isAdmin, isPaidAdmin: !!u.isPaidAdmin }));
}
...
io.on("connection", socket => {
  ...
  socket.on("join", async data => {
    try {
      ...
      const isAdmin = isFirebaseAdminUid(firebaseUid); // Master admin only. Guests can never be admin.
      const paidAdminStatus = firebaseUid
        ? await paidAdminFeature.getStatus(firebaseUid)
      ...
      currentUser = {
        ...
        isAdmin,
        isPaidAdmin: !!paidAdminStatus.active,
        paidAdminExpiresAt: paidAdminStatus.expiresAt || null,
      ...
      socket.emit("history", history.map(m => ({
        ...
        isVip: m.isVip, isPaidAdmin: !!m.isPaidAdmin, createdAt: m.createdAt,
      })));
  ...
  socket.on("chat message", async data => {
    ...
    try {
      ...
      const saved = await safeDB(() => new Message({
        ...
        senderColor: currentUser.color, text: message, type, mediaUrl, caption, replyTo, isVip: currentUser.isVip, isPaidAdmin: !!currentUser.isPaidAdmin,
      }).save(), null);

      const payload = {
        id: saved?._id?.toString() || data.id || genId(), sender: currentUser.name, senderColor: currentUser.color,
        senderAvatar: currentUser.avatar, isVip: currentUser.isVip, isPaidAdmin: !!currentUser.isPaidAdmin, message, type, caption, mediaUrl, replyTo, room, createdAt: saved?.createdAt || new Date(),
      };
  ...
  // ── DELETE MESSAGE ───────────────────────────────────────────────
  // Owners can delete their own messages. Master admins can moderate site-wide.
  // Paid Community Admins are re-verified in MongoDB and can moderate only
  // the room they are currently inside.
  socket.on("delete message", async id => {
    ...
    const messageRoom = message.room || "global";
    const currentRoom = currentUser.room || "global";

    let paidAdminActive = false;
    if (currentUser.firebaseUid) {
      try {
        paidAdminActive = await paidAdminFeature.hasActive(currentUser.firebaseUid);
        currentUser.isPaidAdmin = paidAdminActive;
        if (!paidAdminActive) currentUser.paidAdminExpiresAt = null;
      } catch (err) {
        console.error("Paid admin permission check failed:", err.message);
      }
    }

    const paidAdminCanModerateThisRoom = paidAdminActive && messageRoom === currentRoom;

    if (!ownsMessage && !currentUser.isAdmin && !paidAdminCanModerateThisRoom) {
      return socket.emit("error_msg", "You do not have permission to delete this message.");
    }

    await safeDB(() => Message.findByIdAndDelete(id));
    io.to(messageRoom).emit("delete message", id);
  });

  // Paid role refresh after checkout verification. Master admin state is never changed here.
  socket.on("refresh_paid_admin", async () => {
    if (!currentUser?.firebaseUid) {
      return socket.emit("paid_admin_status", { active: false, expiresAt: null });
    }
    try {
      const status = await paidAdminFeature.getStatus(currentUser.firebaseUid);
      currentUser.isPaidAdmin = !!status.active;
      currentUser.paidAdminExpiresAt = status.expiresAt || null;
      activeUsers[socket.id] = currentUser;
      socket.emit("paid_admin_status", status);
      io.emit("user list", buildUserList());
    } catch (err) {
      socket.emit("paid_admin_status", { active: false, expiresAt: null, error: "Could not refresh Admin Pass" });
    }
  });
  ...
  socket.on("join_group", async ({ groupId, password } = {}) => {
    ...
    try {
      ...
      const normalizedHistory = history.map(m => ({
        ...
        type: m.type||"text", mediaUrl: m.mediaUrl, caption: m.caption||"", replyTo: m.replyTo||null, isVip: m.isVip, isPaidAdmin: !!m.isPaidAdmin, createdAt: m.createdAt,
      }));
  ...
  socket.on("leave_group", async ({ groupId } = {}) => {
    ...
    socket.emit("history", globalHistory.map(m => ({
      id: m._id.toString(), sender: m.senderName, senderColor: m.senderColor, senderAvatar: m.senderAvatar,
      message: m.text, type: m.type||"text", mediaUrl: m.mediaUrl, caption: m.caption||"", replyTo: m.replyTo||null, isVip: m.isVip, isPaidAdmin: !!m.isPaidAdmin, createdAt: m.createdAt,
    })));
  });
  ...
  socket.on("group_message", async data => {
    ...
    try {
      ...
      const payload = { id: saved?._id?.toString() || genId(), groupId, sender: currentUser.name, senderColor: currentUser.color, senderAvatar: currentUser.avatar, isVip: currentUser.isVip, isPaidAdmin: !!currentUser.isPaidAdmin, message, type, mediaUrl, caption, replyTo, createdAt: saved?.createdAt || new Date() };
...
app.get("/guest", (_req, res) => res.redirect(302, "/login?tab=guest"));
...
app.get("/faq", (_req, res) => res.redirect(302, "/"));

app.get("/contact", (_req, res) => res.redirect(302, "/"));
...
app.get("/health", (req, res) => res.json({
  ...
  paidAdminPayments: paidAdminFeature.configured() ? "configured" : "disabled",
}));
...
app.get("/api/admin/emails", requireAdmin, async (req, res) => {
  ...
  // Preferred source: Firebase Authentication. This includes accounts that registered
  // but never completed a profile or joined a chat.
  if (firebaseAdminReady) {
    try {
      const result = await admin.auth().listUsers(limit, pageToken);
      const uids = result.users.map(u => u.uid);
      let profilesByUid = new Map();
      if (mongoConnected && uids.length) {
        const profiles = await safeDB(
          () => UserProfile.find({ firebaseUid: { $in: uids } })
            .select("firebaseUid displayName email createdAt updatedAt")
            .lean(),
          []
        );
        profilesByUid = new Map((profiles || []).map(p => [p.firebaseUid, p]));
      }
      ...
      const users = result.users
        ...
        .map(u => {
          ...
          return {
            ...
            provider: (u.providerData || []).map(p => p.providerId).filter(Boolean).join(", ") || "password",
      ...
      const guestCount = mongoConnected
        ? await safeDB(() => GuestUser.countDocuments(), 0)
        : 0;
      ...
      console.error("Admin email registry Firebase error:", err.message);
      // Continue to MongoDB fallback below.
    }
  }

  // Fallback for deployments where Firebase Admin is not configured. This only
  // contains Firebase users that have already synced a profile to MongoDB.
  if (!mongoConnected) {
    return res.status(503).json({
      error: "Email registry is unavailable. Configure Firebase Admin or MongoDB.",
    });
  }

  const profiles = await safeDB(
    () => UserProfile.find({ email: { $exists: true, $ne: "" } })
      .select("firebaseUid email displayName createdAt updatedAt")
      .sort({ createdAt: -1 })
      .limit(limit)
      .lean(),
    []
  );
  const guestCount = await safeDB(() => GuestUser.countDocuments(), 0);
  return res.json({
    ok: true,
