...
const STATUS_CHANNEL_ID =
  process.env.STATUS_CHANNEL_ID || "1503654154457845900";
const CHAT_CHANNEL_ID = process.env.CHAT_CHANNEL_ID || "1503653808105062480";
const MEDIA_LOG_CHANNEL_ID =
  process.env.MEDIA_LOG_CHANNEL_ID || "1503653995246518292";
const JOIN_LEAVE_CHANNEL_ID =
  process.env.JOIN_LEAVE_CHANNEL_ID || "1503653732372447273";
const MOD_LOG_CHANNEL_ID =
  process.env.MOD_LOG_CHANNEL_ID || "1503653357229969540";
const VIP_LOG_CHANNEL_ID =
  process.env.VIP_LOG_CHANNEL_ID || "1505848329790029844";
const REPORT_CHANNEL_ID =
  process.env.REPORT_CHANNEL_ID || "1505888284927197285";
...
if (fs.existsSync(BANNED_FILE)) {
  try {
    bannedUsernames = new Set(JSON.parse(fs.readFileSync(BANNED_FILE, "utf8")));
  } catch (e) {}
}
if (fs.existsSync(VIPS_FILE)) {
  try {
    vips = new Set(JSON.parse(fs.readFileSync(VIPS_FILE, "utf8")));
  } catch (e) {}
}
...
function saveBanned() {
  ...
  Banned.deleteMany({})
    .then(() => {
      ...
      Array.from(bannedUsernames).forEach((u) =>
        new Banned({ username: u }).save().catch(() => {}),
      );
    })
    .catch(() => {});
}
...
function saveVips() {
  ...
  Vip.deleteMany({})
    .then(() => {
      ...
      Array.from(vips).forEach((u) =>
        new Vip({ username: u }).save().catch(() => {}),
      );
    })
    .catch(() => {});
}
...
const commands = [
  new SlashCommandBuilder()
    .setName("ann")
    .setDescription("Send global announcement")
    .addStringOption((o) =>
      o
        .setName("message")
  ...
  new SlashCommandBuilder()
    .setName("kick")
    .setDescription("Kick user for 5 minutes")
    .addStringOption((o) =>
  ...
  new SlashCommandBuilder()
    .setName("unban")
    .setDescription("Unban a username")
    .addStringOption((o) =>
      o.setName("username").setDescription("Username").setRequired(true),
    ),
  new SlashCommandBuilder()
    .setName("addvip")
    .setDescription("Grant VIP to user")
    .addStringOption((o) =>
      o.setName("username").setDescription("Username").setRequired(true),
    ),
  new SlashCommandBuilder()
    .setName("removevip")
    .setDescription("Revoke VIP status")
    .addStringOption((o) =>
      o.setName("username").setDescription("Username").setRequired(true),
    ),
  new SlashCommandBuilder()
  ...
  new SlashCommandBuilder()
    .setName("cleargroup")
    .setDescription("Clear group messages")
    .addStringOption((o) =>
  ...
  new SlashCommandBuilder()
    .setName("announce")
    .setDescription("Set website live timed announcement")
    .addIntegerOption((o) =>
      o
        .setName("duration")
    ...
    .addStringOption((o) =>
      o
        .setName("message")
        .setDescription("Announcement text paragraph")
        .setRequired(true),
    ),
  new SlashCommandBuilder()
    .setName("active-announcements")
    .setDescription("Check active website announcements"),
  new SlashCommandBuilder()
...
discordClient.once("ready", () => {
  ...
  updateDiscordStatus();
  logToDiscordError(
    `🤖 HeyyYuki Bot Started as ${discordClient.user.tag}`,
    "info",
  );
});
...
discordClient.on("interactionCreate", async (interaction) => {
  ...
  if (!CONTROL_CHANNEL_IDS.includes(interaction.channelId)) {
    return interaction.reply({
      content: `❌ Please use permitted administrator control channels.`,
      ephemeral: true,
    });
  }
  ...
  if (commandName === "announce") {
    ...
    return interaction.editReply(
      `✅ Website par announcement chalu! Ye **${durationInMinutes} minutes** tak dikhegi.`,
    );
  }
  ...
  if (commandName === "remove-currentannouncement") {
    ...
    await Announcement.deleteMany({});
    return interaction.editReply(
      "🗑️ Website se chal rahi announcement ko turant hata diya gaya hai!",
    );
  }
  ...
  if (commandName === "kick") {
    ...
    const sid = Object.keys(activeUsers).find(
      (id) => activeUsers[id]?.name?.toLowerCase() === target,
    );
    if (sid) {
      ...
      io.to(sid).emit("kicked_signal", {
        message: "👢 You have been kicked for 5 minutes.",
        name: u.name,
        expiry,
      });
  ...
  if (commandName === "ban") {
    ...
    sendEmbed(MOD_LOG_CHANNEL_ID, {
      color: 0xff4757,
      title: "🚫 User Banned",
      description: `**${target}** permanently banned.`,
    });
  ...
  if (commandName === "unban") {
    ...
    if (!bannedUsernames.has(target))
      return interaction.reply("❌ Not in ban list.");
    bannedUsernames.delete(target);
    saveBanned();
    sendEmbed(MOD_LOG_CHANNEL_ID, {
      color: 0x2ed573,
      title: "🔓 User Unbanned",
      description: `**${target}** unbanned.`,
    });
  ...
  if (commandName === "addvip") {
    ...
    Object.keys(activeUsers).forEach((sid) => {
      if (activeUsers[sid]?.name?.toLowerCase() === target)
    ...
    sendEmbed(VIP_LOG_CHANNEL_ID, {
      color: 0x1e90ff,
      title: "💎 VIP Granted",
      description: `**${target}** → VIP activated.`,
    });
  ...
  if (commandName === "removevip") {
    ...
    sendEmbed(VIP_LOG_CHANNEL_ID, {
      color: 0xffa500,
      title: "⚠️ VIP Revoked",
      description: `**${target}** → VIP removed.`,
    });
  ...
  if (commandName === "cleargroup") {
    ...
    io.to("group_" + group._id).emit("group_cleared", {
      room: "group_" + group._id,
    });
  ...
  socket.on("join", async (data) => {
    try {
      ...
      if (tempBannedIPs.has(userIP)) {
        ...
        if (Date.now() < ban.expiry) {
          if (nameLower !== ban.reservedName.toLowerCase()) {
            return socket.emit(
              "duplicate",
              `🚫 You are kicked. Only "${ban.reservedName}" allowed from your IP.`,
            );
          }
      ...
      if (duplicate) {
        return socket.emit(
          "duplicate",
          "⚠️ Username already taken. Choose another.",
        );
      }
...
socket.on("delete message", async (id) => {
  try {
    // Try direct MongoDB _id delete
    let deleted = await Message.findByIdAndDelete(id).catch(() => null);
    
    // Agar direct delete fail ho toh text/sender se dhundho
    if (!deleted) {
      deleted = await Message.findOneAndDelete({ 
        $or: [{ _id: id }, { senderId: socket.id }] 
      }).catch(() => null);
    }
    
    // Sabko broadcast karo chahe delete hua ya nahi (UI mein remove karne ke liye)
    io.emit("delete message", id);
    
  } catch(e) {
    // Phir bhi broadcast karo taaki UI update ho
    io.emit("delete message", id);
  }
});
    ...
    if (discordReady) {
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: "📝 Reason",
            value: (data.reason || "—").substring(0, 1000),
            inline: false,
          },
        )
        .setTimestamp()
